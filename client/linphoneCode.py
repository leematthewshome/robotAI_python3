...
are allowed to dial straight through. Unknown callers and those not whitelisted are blocked.
...
import sqlite3
import threading
#allow for running either in isolation or via robotAI.py
try:
...
class SecurityCamera:
    ...
    def __init__(self, ENVIRON, SENSORQ, MIC, snd_playback=''):
        ...
        if os.path.isfile(filename):
            config = app_utils.getConfigData(self.TOPDIR, "Phone")
            if "ERROR" in config:
                logger.debug("Error getting the sensor configuration: " + config["ERROR"])
            else:
                debugFlag = app_utils.getConfig(config, "Phone_2debug")
                self.camera = app_utils.getConfig(config, "Phone_camera")
                self.snd_capture = app_utils.getConfig(config, "Phone_snd_capture")
                self.admin = app_utils.getConfig(config, "Phone_admin")
                username = app_utils.getConfig(config, "Phone_account")
                password = app_utils.getConfig(config, "Phone_password")
    ...
    def call_state_changed(self, core, call, state, message):
        ...
        if state == linphone.CallState.Idle:
            self.MIC.say("The call state is now Idle.")
        ...
        if state == linphone.CallState.Error:
            ...
            if self.outgoingCall == True:
                self.MIC.say("Sorry, I was not able to connect your call.")
                self.MIC.say("The other party may not be connected to the server.")
            else:
                self.MIC.say("There was some sort of error with the call.")
        ...
        if state == linphone.CallState.End:
            ...
            self.MIC.say("The call has ended.")
            self.ENVIRON["listen"] = True
...
def doSensor(ENVIRON, SENSORQ, MIC):
    phoneSensor = SecurityCamera(ENVIRON, SENSORQ, MIC)
    phoneSensor.run()
...
if __name__ == "__main__":
    print("******** WARNING ********** Starting PhoneSensor from __main__ procedure")
